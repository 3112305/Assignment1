...
void printMessage(char); // reads message file and prints contents send filename, return message
...
int main(){
    ...
    if(progSelection1 == 1){// Cipher
       ...
       if(progSelection2 == 1){
          ...
          filename[0] = "RotationCipher.txt";//set filename
          message[0] = fileRead(filename);// run fileRead function to read messagefrom file
          rotKey = getRotKey(); // then get rotation key from file // run function to get the rotation key
          message[0] = rotateKey(message,rotKey); // then rotate message with key
